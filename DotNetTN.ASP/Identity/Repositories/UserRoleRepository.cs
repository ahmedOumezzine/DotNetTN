            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                if (!string.IsNullOrEmpty(roleId))
                {
                    using (var conn1 = new MySqlConnection(_connectionString))
                    {
                        ...
                        MySqlHelper.ExecuteNonQuery(conn1, @"Insert into identity_usersroles(UserId,RoleId) VALUES(@Id,@RoleId)", parameters1);
                    }
                }
            }
        }
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                if (!string.IsNullOrEmpty(roleId))
                {
                    using (var conn1 = new MySqlConnection(_connectionString))
                    {
                        ...
                        MySqlHelper.ExecuteNonQuery(conn1, @"Delete FROM identity_usersroles WHERE UserId=@Id AND RoleId=@RoleId", parameters1);
                    }
                }
            }
        }
        ...
        public List<String> PopulateRoles(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                    @"SELECT RoleId FROM identity_usersroles Where UserId=@Id", parameters);
                while (reader.Read())
                {
