    ...
    public class UserClaimRepository<TUser> where TUser : IdentityUser
    {
        ...
        public void Insert(TUser user, Claim claim)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn, @"INSERT INTO identity_userclaims(UserId,ClaimType,ClaimValue) VALUES(@UserId,@ClaimType,@ClaimValue)", parameters);
            }
        }
        ...
        public void Delete(TUser user, Claim claim)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                MySqlHelper.ExecuteNonQuery(conn,
                    @"DELETE FROM identity_userclaims WHERE UserId=@UserId AND ClaimType=@ClaimType AND ClaimValue=@ClaimValue", parameters);
            }
        }
        ...
        public List<IdentityUserClaim> PopulateClaims(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = MySqlHelper.ExecuteReader(conn, CommandType.Text,
                    @"SELECT ClaimType,ClaimValue FROM identity_userclaims WHERE UserId=@Id", parameters);
                while (reader.Read())
                {
